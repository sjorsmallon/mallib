...
#include "../../framework/io/io.h" // Keyboard_State, Platform_Key_Map 
#include "../../common/globals.h"  // globals.window_width, globals.window_height
...
// private
static void insert_input_in_queue(std::vector<io::Button>& input_queue, io::Keyboard_State& prev_keyboard_state, io::Keyboard_State& current_keyboard_state)
{
    ...
    std::swap(current_keyboard_state, prev_keyboard_state);

    for (auto key: keys)
    {
        if(GetAsyncKeyState(key))
        {
            current_keyboard_state[key] = true;
            ...
            current_keyboard_state[key] = false;
    }
}
                    ...
                    int command_show)
{
    ...
    //@Global
    win32_globals.device_context = device_context;
    win32_globals.gl_context = gl_context;
    win32_globals.window_handle = window;
    ...
    {
        // change window size
        const int width =  1920; // @fixme: this is hardcoded.
        ...
        while (running)
        {
            ...
            insert_input_in_queue(io::input_queue(), io::keyboard_state(), io::prev_keyboard_state());
            application_loop();
        }               
    }
